 ...
 * waiting on relation extension lock, page locks or speculative locks.
 *
...
IsProcessWaitingForSafeOperations(PGPROC *proc)
{
	...
	return waitLock->tag.locktag_type == LOCKTAG_RELATION_EXTEND ||
		   ...
		   waitLock->tag.locktag_type == LOCKTAG_TUPLE;
}
...
AddEdgesForLockWaits(WaitGraph *waitGraph, PGPROC *waitingProc, PROCStack *remaining)
{
	...
	while (procLock != NULL)
	{
		 ...
		 * and processes that are waiting on a relation extension lock, page locks
		 * or speculative locks which will be released shortly.
		 */
