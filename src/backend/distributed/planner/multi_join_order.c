			  ...
			  List *applicableJoinClauses, JoinType joinType)
{
	...
	/*
	 * If the table is a reference table, then we assume reference join is feasible.
	 * This assumption is valid only for inner joins.
	 *
	 * Right join requires existing (left) table to have single shard, full outer
	 * join requires both tables to have single shard.
	 */
