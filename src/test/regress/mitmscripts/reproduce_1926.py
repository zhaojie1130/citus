...
def tcp_message(flow):
    ...
    # 3. if we've marked a flow to be killed, kill it!
    if tcp_msg.from_client and getattr(flow, 'should_kill', False):
        flow.kill()

    # 2. Look for SELECT. If you find it... kill after the next message
    if tcp_msg.from_client and b'SELECT' in tcp_msg.content:
